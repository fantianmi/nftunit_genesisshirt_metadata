...
//        array('password', '6,30', -4, self::EXISTS_VALIDATE, 'length'), //密码长度不合法
    ...
    protected function checkUsernameLength($username)
    {
        ...
        if ($length < modC('USERNAME_MIN_LENGTH', 2, 'USERCONFIG') || $length > modC('USERNAME_MAX_LENGTH', 256, 'USERCONFIG')) {
            return false;
        }
        return true;
    }
    ...
    public function login($username, $password, $type = 1)
    {
        ...
        if (UC_SYNC && $user['id'] != 1 && file_exists('./api/config.php')) {
    ...
    public function loginWithAddress($username)
    {
        $map['username'] = strtolower($username);
        /* 获取用户数据 */
        $user = $this->where($map)->find();
        if (is_array($user) && $user['status']) {
            /* 验证用户密码 */
    ...
    public function updatePassword($new_password, $uid = 0)
    {
        if (!$uid) {
            $uid = get_uid();
        }
     ...
     * @auth 陈一枭
     */
    public function changePassword($old_password, $new_password, $uid = 0)
    {
        if (intval($uid) <= 0) {
            $uid = get_uid();
        }
        ...
        if (!$this->verifyUser($uid, $old_password)) {
            $this->error = -41;
            return false;
        }
        //更新用户信息
    ...
    {
        $email = create_rand(10) . modC('SYNC_LOGIN_EMAIL_SUFFIX', '@ocenter.com', 'USERCONFIG');
