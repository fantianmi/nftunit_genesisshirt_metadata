...
1600:Lv7 '.L('_CHAIRMAN__').'
    ...
    public function edit_expandinfo($profile_group_id)
    {
        //同时修改member表生日字段值
        $uid = is_login();
        $birthId = M('field_setting')->where(array('field_name' => '生日'))->getField('id');
        $birthday = text($_POST['expand_' . $birthId]);
        D('Member')->where(array('uid' => $uid))->setField('birthday', $birthday);
    ...
    public function rankVerifyCancel($rank_id = null)
    {
        ...
        if (is_login() && $rank_id) {
            ...
            if ($result) {
                ...
                $this->success(L('_SUCCESS_CANCEL_'), U('Ucenter/Config/rankVerifyWait'));
            } else {
    ...
    public function verifyy($rank_id = null, $reason = null, $rank_user_id = 0)
    {
        $rank_id = intval($rank_id);
        $reason = op_t($reason);
        $rank_user_id = intval($rank_user_id);
        ...
        if ($result) {
            ...
            $this->success(L('_SUCCESS_RANK_APPLY_'), U('Ucenter/Config/rankVerify'));
        } else {
