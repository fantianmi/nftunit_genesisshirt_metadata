...
class MemberController extends Controller
{
    ...
    public function mi($email = '', $verify = '')
    {
        ...
        if (IS_POST) { //登录验证
            ...
            $content = C('USER_RESPASS') . "<br/>" . $url . "<br/>" . modC('WEB_SITE_NAME', L('_kmCms_'), 'Config') . L('_SEND_MAIL_AUTO_') . "<br/>" . date('Y-m-d H:i:s', TIME()) . "</p>";
            send_mail($email, modC('WEB_SITE_NAME', L('_kmCms_'), 'Config') . L('_SEND_MAIL_PASSWORD_FOUND_'), $content);
    ...
    public function doSendVerify($account, $verify, $type,$type_other)
    {
        switch ($type) {
            ...
            case 'email':
                ...
                $content = str_replace('{$verify}', $verify, $content);
                $content = str_replace('{$account}', $account, $content);
                $res = send_mail($account, modC('WEB_SITE_NAME', L('_kmCms_'), 'Config') . L('_EMAIL_VERIFY_2_'), $content);
                return $res;
                break;
        }
    ...
    private function sendActivateEmail($account, $verify, $uid)
    {
        ...
        $content = str_replace('{$title}', modC('WEB_SITE_NAME', L('_kmCms_'), 'Config'), $content);
        $res = send_mail($account, modC('WEB_SITE_NAME', L('_kmCms_'), 'Config') . L('_VERIFY_LETTER_'), $content);
