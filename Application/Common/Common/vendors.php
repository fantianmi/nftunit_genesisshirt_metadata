...
{
    $site_name = modC('WEB_SITE_NAME', L('_kmCms_OPEN_SOURCE_SOCIAL_SYSTEM_'), 'Config');
    if ($to == '') {
...
function send_mail_local($to = '', $subject = '', $body = '', $name = '', $attachment = null)
{
    ...
    $from_name = modC('WEB_SITE_NAME', L('_kmCms_OPEN_SOURCE_SOCIAL_SYSTEM_'), 'Config');
    $reply_email = '';
    $reply_name = '';
    ...
    if ($name == '') {
        $name = modC('WEB_SITE_NAME', L('_kmCms_OPEN_SOURCE_SOCIAL_SYSTEM_'), 'Config'); //发送者名称为空时，默认使用网站名称
    }
    if ($subject == '') {
        $subject = modC('WEB_SITE_NAME', L('_kmCms_OPEN_SOURCE_SOCIAL_SYSTEM_'), 'Config'); //邮件主题为空时，默认使用网站标题
    }
    if ($body == '') {
        $body = modC('WEB_SITE_NAME', L('_kmCms_OPEN_SOURCE_SOCIAL_SYSTEM_'), 'Config'); //邮件内容为空时，默认使用网站描述
    }
...
function thinkox_hash($message, $salt = "kmCms")
{
    $s01 = $message . $salt;
    $s02 = md5($s01) . $salt;
