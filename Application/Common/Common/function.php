...
require_once(APP_PATH . '/Common/Common/common_tools.php');
require_once(APP_PATH . '/Common/Common/redis.php');
require_once(APP_PATH . '/Common/Common/sms.php');
require_once(APP_PATH . '/Common/Common/entrust.php');
require_once(APP_PATH . '/Common/Common/wallet.php');
...
function get_wallet_address($uid = null)
{
    $user = query_user('username', $uid);
    return $user['username'];
}

function get_brain_token($uid)
{
    return D("Member")->where(['uid' => $uid])->getField("brain_token");
}

function get_total_brain($uid)
{
    $total = S("get_total_brain_{$uid}");
    if (!$total) {
        $base = get_brain_token($uid);
        $result = D("AirdropDetail")->field("SUM(amount) total")->where(['uid' => $uid, 'status' => 1])->find();
        $total = $base + $result['total'];
        S("get_total_brain_{$uid}", $total, 600);
    }
    return $total;
...
function get_nftunit_id($uid = null)
{
    $user = query_user('nftunit_id', $uid);
    return $user['nftunit_id'];
}

function get_airdrop_title($aid)
{
    return D("Airdrop")->where(['id' => $aid])->getField("title");
}

function get_txid($aid)
{
    $result = D("Airdrop")->where(['id' => $aid])->getField("result_api");
    $txid = "";
    if ($result) {
