...
abstract class Controller
{
    ...
    /**
     * _firstUserRun 发起每日第一位访问的用户执行计划（用于执行数据统计、连签重置等操作）
     * @return bool
    ...
    private function _firstUserRun()
    {
        ...
        if(intval($first_user_run_time)+60<time()){
            ...
            if($lastRunTime!=$today){
                ...
                $time = time();
                $url = U('Core/Public/firstUserRun', array('time' => $time, 'token' => md5($time . C('DATA_AUTH_KEY'))), true, true);
                $ch = curl_init();
